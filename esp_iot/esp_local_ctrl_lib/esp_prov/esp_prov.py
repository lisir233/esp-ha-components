...
# Import from local relative paths
from . import security
# from . import prov  # TODO: WiFi provisioning module not yet imported - needed for WiFi prov functions
    ...
    try:
        ...
        if sel_transport in ["softap", "httpd"]:
            ...
            tp = transport.Transport_BLE(
                service_uuid="021a9004-0382-4aea-bff4-6b3f1c5adfb4", nu_lookup=nu_lookup
            )
...
async def version_match(tp, protover, verbose=False):
    try:
        response = await tp.send_data_and_receive("proto-ver", protover)
...
async def has_capability(tp, capability="none", verbose=False):
    ...
    try:
        response = await tp.send_data_and_receive("proto-ver", capability)
...
async def get_version(tp):
    response = None
    try:
        response = await tp.send_data_and_receive("proto-ver", "---")
    except RuntimeError as e:
        on_except(e)
        response = ""
    return response
...
async def establish_session(tp, sec):
    try:
        ...
        while True:
            ...
            if request is None:
                break
            response = await tp.send_data_and_receive("prov-session", request)
            if response is None:
                return False
        return True
    except RuntimeError as e:
...
async def custom_config(tp, sec, custom_info, custom_ver):
    try:
        ...
        response = await tp.send_data_and_receive("custom-config", message)
...
async def custom_data(tp, sec, custom_data):
    try:
        ...
        response = await tp.send_data_and_receive("custom-data", message)
...
async def scan_wifi_APs(sel_transport, tp, sec):
    ...
    try:
        ...
        response = await tp.send_data_and_receive("prov-scan", message)
        stop_time = time.time()
...
async def send_wifi_config(tp, sec, ssid, passphrase):
    try:
        ...
        response = await tp.send_data_and_receive("prov-config", message)
...
async def reset_wifi(tp, sec):
    try:
        ...
        response = await tp.send_data_and_receive("prov-ctrl", message)
    ...
    try:
        ...
        if args.ssid == "":
            if not await has_capability(obj_transport, "wifi_scan"):
                raise RuntimeError(
                    "Wi-Fi Scan List is not supported by provisioning service"
                )

            while True:
                print("\n==== Scanning Wi-Fi APs ====")
                start_time = time.time()
                APs = await scan_wifi_APs(
                    args.mode.lower(), obj_transport, obj_security
                )
                end_time = time.time()
                ...
                print(
                    "{0: >4} {1: <33} {2: <12} {3: >4} {4: <4} {5: <16}".format(
                        "S.N.", "SSID", "BSSID", "CHN", "RSSI", "AUTH"
                    )
                )
                for i in range(len(APs)):
                    print(
                        "[{0: >2}] {1: <33} {2: <12} {3: >4} {4: <4} {5: <16}".format(
                            i + 1,
                            APs[i]["ssid"],
                            APs[i]["bssid"],
                            APs[i]["channel"],
        ...
        if args.passphrase is None:
            prompt_str = f"Enter passphrase for {args.ssid} : "
            args.passphrase = getpass(prompt_str)

        print("\n==== Sending Wi-Fi Credentials to Target ====")
        if not await send_wifi_config(
            obj_transport, obj_security, args.ssid, args.passphrase
        ):
